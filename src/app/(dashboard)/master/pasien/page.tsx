...
export default function PasienPage() {
  ...
  const [pasienData, setPasienData] = useState<Pasien[]>(() => {
    // Sort by noRM descending (highest number first)
    return [...pasienList].sort((a, b) => {
      // Extract number from RM-YYYY-XXXX format
      const numA = parseInt(a.noRM.split('-').pop() || '0');
      const numB = parseInt(b.noRM.split('-').pop() || '0');
      return numB - numA;
    });
  });
  ...
  const filteredPasien = pasienData
    .filter(
      (p) =>
    ...
    // Removed duplicate sorting since data is already sorted in state
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedPasien) {
      ...
      const newPasien: Pasien = {
        id: `PAS${String(pasienData.length + 1).padStart(3, '0')}`,
        noRM: `RM-${currentYear}-${String(pasienData.length + 1).padStart(4, '0')}`,
        createdAt: new Date(),
        updatedAt: new Date(),
        ...updatedFormData as Omit<Pasien, 'id' | 'noRM' | 'createdAt' | 'updatedAt'>,
      };
