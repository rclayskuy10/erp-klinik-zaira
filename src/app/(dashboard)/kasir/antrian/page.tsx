...
export default function AntrianPage() {
  ...
  const [localRegistrasi, setLocalRegistrasi] = useState(registrasiList);
  
  const antrianHariIni = localRegistrasi.filter(r => {
  ...
  const handlePanggil = (id: string, nomorAntrian: number) => {
    // Update status menjadi diperiksa
    setLocalRegistrasi(prev => prev.map(reg => {
      if (reg.id === id) {
        return { ...reg, status: 'diperiksa' as const };
      }
      // Set antrian lain di poli yang sama menjadi menunggu jika sedang diperiksa
      if (reg.poliId === prev.find(r => r.id === id)?.poliId && reg.status === 'diperiksa') {
        return { ...reg, status: 'menunggu' as const };
      }
      return reg;
    }));
    
    // Play sound or announcement
    alert(`Nomor antrian ${nomorAntrian} silakan ke ruangan pemeriksaan`);
  };

  const handleSelesai = (id: string) => {
    // Update status menjadi selesai
    setLocalRegistrasi(prev => prev.map(reg => 
      reg.id === id ? { ...reg, status: 'selesai' as const } : reg
    ));
  };
  ...
  const handleBatal = (id: string) => {
    if (confirm('Apakah Anda yakin ingin membatalkan antrian ini?')) {
      ...
      setLocalRegistrasi(prev => prev.map(reg => 
        reg.id === id ? { ...reg, status: 'batal' as const } : reg
      ));
    }
  };
                          ...
                          {reg.status === 'menunggu' && (
                            <>
                              <Button
                                type="button"
                                size="sm"
                                onClick={() => handlePanggil(reg.id, reg.nomorAntrian)}
                                className="gap-1"
