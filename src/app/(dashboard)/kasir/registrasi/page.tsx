...
export default function RegistrasiPage() {
  ...
  const handleNewPatientSubmit = (e: React.FormEvent) => {
    ...
    // Validasi
    if (!newPatientData.nik || !newPatientData.nama || !newPatientData.tanggalLahir || 
        !newPatientData.jenisKelamin || !newPatientData.telepon || !newPatientData.alamat) {
    ...
    const newPasien: Pasien = {
      id: generateId('PAS'),
      noRM: `RM-2026-${String(pasienList.length + 1).padStart(4, '0')}`,
      nik: newPatientData.nik,
      noBPJS: newPatientData.noBPJS || undefined,
      nama: newPatientData.nama,
      tanggalLahir: new Date(newPatientData.tanggalLahir),
      jenisKelamin: newPatientData.jenisKelamin as 'L' | 'P',
      alamat: newPatientData.alamat,
      telepon: newPatientData.telepon,
      email: newPatientData.email || undefined,
      createdAt: new Date(),
      updatedAt: new Date(),
    };

    // Tambahkan ke list (dalam implementasi real, ini akan ke API)
    pasienList.unshift(newPasien);
    
    // Pilih pasien baru ini dan lanjut ke form
        ...
        <form onSubmit={handleNewPatientSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input 
              label="Tanggal Lahir" 
              type="date" 
              value={newPatientData.tanggalLahir}
              onChange={(e) => setNewPatientData({...newPatientData, tanggalLahir: e.target.value})}
              required
            />
            <Select
              label="Jenis Kelamin"
              value={newPatientData.jenisKelamin}
              onChange={(e) => setNewPatientData({...newPatientData, jenisKelamin: e.target.value})}
              options={[
            ...
            <Input 
              ...
              value={newPatientData.telepon}
              onChange={(e) => setNewPatientData({...newPatientData, telepon: e.target.value})}
              required
            />
            <Input 
            ...
            <Input 
              ...
              value={newPatientData.noBPJS}
              onChange={(e) => setNewPatientData({...newPatientData, noBPJS: e.target.value})}
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
