...
export default function LaporanStokPage() {
  ...
  // Handle export to Excel (CSV format) - Movement
  const handleExportToExcel = () => {
    const dataToExport = activeTab === 'movement' ? filteredMovement : stokSummary;
    
    if (activeTab === 'movement') {
      const headers = ['Tanggal', 'Kode Item', 'Nama Item', 'Tipe', 'Jenis', 'Kategori', 'Jumlah', 'Satuan', 'Stok Awal', 'Stok Akhir', 'Keterangan'];
      const rows = filteredMovement.map(item => [
      ...
      link.setAttribute('download', `laporan_stok_movement_${timestamp}.csv`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    } else {
      const headers = ['Kode', 'Nama Item', 'Tipe', 'Stok Awal', 'Masuk', 'Keluar', 'Stok Akhir', 'Satuan', 'Nilai Stok'];
      const rows = stokSummary.map(item => [
        item.kode,
        item.nama,
        item.tipe === 'obat' ? 'Obat' : 'Alat Medis',
        item.stokAwal,
      ...
      const timestamp = new Date().toISOString().replace(/[-:]/g, '').replace('T', '_').split('.')[0];
      link.setAttribute('href', url);
      link.setAttribute('download', `laporan_stok_summary_${timestamp}.csv`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const handleExportToPDF = () => {
    const printContent = activeTab === 'movement' ? `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          ...
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .badge { display: inline-block; padding: 2px 6px; border-radius: 10px; font-size: 7pt; }
      ...
      <body>
        ...
        <div class="info">
          ...
          <p><strong>Total Masuk:</strong> ${totalMasuk} item | <strong>Total Keluar:</strong> ${totalKeluar} item</p>
          ...
          <tbody>
            ${filteredMovement.map(item => `
              <tr>
                ...
                <td>${item.kodeItem}</td>
                <td>${item.namaItem}</td>
                <td>${item.tipe === 'obat' ? 'Obat' : 'Alat'}</td>
                <td class="text-center"><span class="badge badge-${item.jenis}">${item.jenis === 'masuk' ? 'Masuk' : 'Keluar'}</span></td>
                <td>${item.kategori}</td>
                <td class="text-center">${item.jenis === 'masuk' ? '+' : '-'}${item.jumlah} ${item.satuan}</td>
                <td class="text-center">${item.stokAwal} → ${item.stokAkhir}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      ...
      <body>
        ...
        <div class="summary">
          ...
          <div class="summary-item">
            ...
            <div class="value">${formatCurrency(totalNilaiStok)}</div>
          </div>
          <div class="summary-item">
            <div class="label">Stok Rendah</div>
            <div class="value">${itemStokRendah}</div>
          </div>
        </div>
        <table>
          <thead>
          ...
          <tbody>
            ${stokSummary.map(item => `
              <tr>
                <td>${item.kode}</td>
                <td>${item.nama}</td>
                <td>${item.tipe === 'obat' ? 'Obat' : 'Alat Medis'}</td>
                <td class="text-center">${item.stokAwal}</td>
                <td class="text-center">${item.masuk}</td>
                <td class="text-center">${item.keluar}</td>
                <td class="text-center">${item.stokAkhir}</td>
                <td class="text-right">${formatCurrency(item.nilaiStok)}</td>
              </tr>
            `).join('')}
            <tr style="font-weight: bold; background-color: #e0f2fe;">
              <td colspan="7" class="text-right">TOTAL NILAI STOK</td>
              <td class="text-right">${formatCurrency(totalNilaiStok)}</td>
            </tr>
          </tbody>
        </table>
        <div class="footer">
