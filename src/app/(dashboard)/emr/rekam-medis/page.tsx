...
function RekamMedisContent() {
  ...
  const generatePrintContent = (rm: RekamMedis) => {
    const pasien = getPasienById(rm.pasienId);
    ...
    return `
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .header h2 { font-size: 16px; margin-top: 15px; }
          .header p { font-size: 11px; color: #333; }
          .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 20px; padding: 10px; background: #f9f9f9; }
          .info-item { display: flex; }
          .info-label { width: 120px; font-weight: 600; }
          .section { margin-bottom: 20px; }
          .section-title { font-size: 14px; font-weight: 600; border-bottom: 1px solid #ccc; padding-bottom: 5px; margin-bottom: 10px; }
          .soap-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
          .soap-box { padding: 10px; border: 1px solid #ddd; border-radius: 4px; }
          .soap-box h4 { font-size: 12px; font-weight: 600; margin-bottom: 5px; }
          table { width: 100%; border-collapse: collapse; margin-top: 10px; }
          th, td { border: 1px solid #000; padding: 8px; text-align: left; }
          th { background: #f3f4f6; font-weight: 600; }
          .text-right { text-align: right; }
          .diagnosis-item { padding: 5px 0; border-bottom: 1px solid #eee; }
          .badge { display: inline-block; padding: 2px 8px; font-size: 10px; border-radius: 4px; margin-right: 8px; }
      ...
      <body>
        ...
        <div class="info-grid">
          <div class="info-item"><span class="info-label">No. RM</span>: ${pasien?.noRM}</div>
          <div class="info-item"><span class="info-label">Tanggal</span>: ${formatDate(rm.tanggal)}</div>
          <div class="info-item"><span class="info-label">Nama Pasien</span>: ${pasien?.nama}</div>
          <div class="info-item"><span class="info-label">Dokter</span>: ${dokter?.nama}</div>
          <div class="info-item"><span class="info-label">ID Kunjungan</span>: ${rm.kunjunganId}</div>
          <div class="info-item"><span class="info-label">Jenis Kelamin</span>: ${pasien?.jenisKelamin === 'L' ? 'Laki-laki' : 'Perempuan'}</div>
        </div>
        ...
        <div class="section">
          ...
          <div class="soap-grid">
            <div class="soap-box">
              <h4>Subjective</h4>
              <p>${rm.soap.subjective}</p>
            </div>
            <div class="soap-box">
              <h4>Objective</h4>
              <p>
                ${rm.soap.objective.tekananDarah ? `TD: ${rm.soap.objective.tekananDarah} mmHg<br>` : ''}
                ${rm.soap.objective.suhu ? `Suhu: ${rm.soap.objective.suhu}°C<br>` : ''}
                ${rm.soap.objective.nadi ? `Nadi: ${rm.soap.objective.nadi}x/menit<br>` : ''}
                ${rm.soap.objective.pernapasan ? `RR: ${rm.soap.objective.pernapasan}x/menit<br>` : ''}
                ${rm.soap.objective.beratBadan ? `BB: ${rm.soap.objective.beratBadan} kg<br>` : ''}
                ${rm.soap.objective.pemeriksaanFisik || ''}
              </p>
            </div>
            <div class="soap-box">
              <h4>Assessment</h4>
              <p>${rm.soap.assessment}</p>
            </div>
            <div class="soap-box">
              <h4>Plan</h4>
              <p>${rm.soap.plan}</p>
        ...
        <div class="section">
          ...
          ${rm.diagnosis.map(d => `
            <div class="diagnosis-item">
              <span class="badge ${d.tipe === 'utama' ? 'badge-primary' : 'badge-secondary'}">${d.tipe === 'utama' ? 'Utama' : 'Sekunder'}</span>
              <strong>${d.kodeICD10}</strong> - ${d.nama}
            </div>
          `).join('')}
        </div>
            ...
            <tbody>
              ${rm.tindakan.map(t => `
                <tr>
                  <td>${t.namaLayanan}</td>
                  <td>${t.jumlah}</td>
                  <td class="text-right">${formatCurrency(t.tarif)}</td>
                  <td class="text-right">${formatCurrency(t.subtotal)}</td>
                </tr>
              `).join('')}
            </tbody>
            <tfoot>
              <tr>
                ...
                <td class="text-right"><strong>${formatCurrency(rm.tindakan.reduce((sum, t) => sum + t.subtotal, 0))}</strong></td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div class="section">
          ...
          <p style="padding: 10px; background: #f9f9f9; border-radius: 4px;">${rm.catatan}</p>
        </div>
        ` : ''}
        
        <div class="footer">
          <div class="signature">
            <p>Kota Sehat, ${formatDate(rm.tanggal)}</p>
  ...
  const handlePrint = (rm: RekamMedis) => {
    const printContent = generatePrintContent(rm);
  ...
  const handleDownloadPDF = (rm: RekamMedis) => {
    const printContent = generatePrintContent(rm);
    ...
    if (iframeDoc) {
      ...
      iframe.onload = () => {
        iframe.contentWindow?.focus();
        iframe.contentWindow?.print();
        // Don't remove iframe immediately for PDF save
      };
    }
  };
        ...
        {selectedRM && (
          <div className="space-y-6">
            ...
            <div className="hidden print:block text-center mb-6 pb-4 border-b-2 border-black">
              <h1 className="text-2xl font-bold">KLINIK ZAIRA</h1>
              <p className="text-sm mt-1">Jl. Kesehatan No. 123, Kota Sehat</p>
              <p className="text-sm">Telp: (021) 1234567 | Email: info@klinikzaira.com</p>
              <h2 className="text-xl font-semibold mt-4 mb-6">REKAM MEDIS ELEKTRONIK</h2>
              
              {/* Informasi Pasien untuk Print */}
              <div className="text-left mt-4 space-y-1">
                <div className="grid grid-cols-2 gap-x-8 text-sm">
                  <div className="flex">
                    <span className="w-32 font-medium">No. RM</span>
                    <span>: {getPasienById(selectedRM.pasienId)?.noRM}</span>
                  </div>
                  <div className="flex">
                    <span className="w-32 font-medium">Tanggal</span>
                    <span>: {formatDate(selectedRM.tanggal)}</span>
                  </div>
                  <div className="flex">
                    <span className="w-32 font-medium">Nama Pasien</span>
                    <span>: {getPasienById(selectedRM.pasienId)?.nama}</span>
                  </div>
                  <div className="flex">
                    <span className="w-32 font-medium">Dokter</span>
                    <span>: {getDokterById(selectedRM.dokterId)?.nama}</span>
                  </div>
                  <div className="flex">
                    <span className="w-32 font-medium">ID Kunjungan</span>
                    <span>: {selectedRM.kunjunganId}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Header Info */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg print:bg-white">
              <div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              <Button 
                variant="outline"
                onClick={() => handlePrint(selectedRM)}
              >
                <Printer className="w-4 h-4" />
                Cetak
              </Button>
              <Button
                onClick={() => handleDownloadPDF(selectedRM)}
              >
