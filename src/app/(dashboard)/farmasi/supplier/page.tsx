...
export default function SupplierPage() {
  ...
  const handleSubmit = () => {
    // Validasi
    if (!formData.nama || !formData.alamat || !formData.telepon || !formData.email || !formData.kontakPerson) {
      alert('Mohon lengkapi semua field yang wajib diisi!');
      return;
    }

    if (isEditing && selectedSupplier) {
      // Update supplier existing
      setSuppliers(suppliers.map(sup => 
        sup.id === selectedSupplier.id ? { ...sup, ...formData } as Supplier : sup
      ));
